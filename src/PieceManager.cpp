...
PieceManager::PieceManager(
    ...
    const int maximumConnections) : fileParser(fileParser), maximumConnections(maximumConnections), pieceLength(fileParser.getPieceLength())
{

    // Creates the destination file with the file size specified in the Torrent file
    downloadedFile.open(downloadPath, std::ios::binary | std::ios::in | std::ios::out);
    if (!downloadedFile.is_open())
    {
        downloadedFile.open(downloadPath, std::ios::binary | std::ios::out);
        std::cout << "Downloading first time" << std::endl;
        for (long i = 0; i < fileParser.getFileSize(); ++i)
        {
            downloadedFile.put('#');
        }
    ...
    {
        std::cout << "Downloading missing pieces" << std::endl;
        char ch;
        long index = 0;
        downloadedFile.seekg(0, std::ios::beg);
        downloadedFile.seekp(0, std::ios::beg);
        while (downloadedFile.get(ch))
        {
            std::cout << ch
                      << " ";
            if (ch != '#' && index % fileParser.getPieceLength() == 0)
            {

                std::cout << index / fileParser.getPieceLength() << " ";
                havealready.insert(index / fileParser.getPieceLength());
            }
            ++index;
        }
        std::cout << " \n Number of pieces already present = " << havealready.size() << std::endl;
    }
    auto it = initiatePieces();

    missingPieces = it.first;
    havePieces = it.second;
    // downloadedFile.seekp(fileParser.getFileSize() - 1);
    // downloadedFile.write("", 1);
...
//  void PieceManager::finalizeAndExit() {
...
//         delete this;
//     }
//     PieceManager *pieceManagerInstance = nullptr;

// // Signal handler function
// void signalHandler(int signal) {
//     if (signal == SIGINT && pieceManagerInstance != nullptr) {
//         // Call finalizeAndExit on the PieceManager instance
//         pieceManagerInstance->finalizeAndExit();
//     }
// }
/**
 ...
 */
std::pair<std::vector<Piece *>, std::vector<Piece *>> PieceManager::initiatePieces()
{
    std::vector<std::string> pieceHashes = fileParser.splitPieceHashes();
    totalPieces = pieceHashes.size();
    std::vector<Piece *> torrentPieces;
    if (havealready.size() < totalPieces)
        missingPieces.reserve(totalPieces - havealready.size());
    if (havealready.size() > 0)
        havePieces.reserve(havealready.size());
    ...
    return {torrentPieces, havePieces};
}
...
void PieceManager::addPeer(const std::string &peerId, std::string bitField)
{
    ...
    info << "Number of connections: " << std::to_string(peers.size()) << "/" + std::to_string(maximumConnections);
...
{
    std::cout << "writing piece " << piece->index << std::endl;
